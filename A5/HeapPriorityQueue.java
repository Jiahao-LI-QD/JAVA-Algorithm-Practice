...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  protected void upheapRec( int j ) {
	...
	// Use recursion.  Do NOT use any kind of loop. 
	  if (j == 1) {
		  ...
		  int i = this.parent(j);
		  if (compare(this.heap.get(i), this.heap.get(j)) < 0) {
			  return;
		  }
		  else {
			  swap(i, j);
  ...
  protected void downheapRec( int j ) {
	  ...
	  if (!this.hasRight(j)) {
		  smallindex = this.left(j);
	  }
	  else {
		  smallindex = this.compare(this.heap.get(this.left(j)), this.heap.get(this.right(j))) <= 0?
				  this.left(j) : this.right(j);
	  }
	  if (this.compare(this.heap.get(smallindex), this.heap.get(j)) <  0) {
		  swap (smallindex, j);
		  this.downheapRec(smallindex);
	  }
	  else {
		  return;
	  }
  ...
  public int updateKey( K currentKey, K newKey ) {
	...
	// You may use loops in this method.
	  int index = 0;
	  for (int i = 1; i < heap.size() ; i++) {
		  if (this.heap.get(i).getKey() == currentKey) {
			  index = i;
		  }
	  }
	  if (index == 0) {
	  ...
	  Entry<K, V> pre = this.heap.set(index, new PQEntry<>(newKey ,this.heap.get(index).getValue()));
	  Entry<K, V> now = this.heap.get(index);
	  if (compare(pre, this.heap.get(index)) < 0) {
		  this.downheapRec(index);
	  }
	  else {
		  this.upheapRec(index);
	  }
      return this.heap.indexOf(now);   // replace this line with your code
